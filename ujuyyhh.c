...
int main(int argc, char** argv) {
    ...
    for (i=1; i<argc; i++) {
        ...
        if (TESTFLAG) {
            double x = root(g, t, -100, 100, EPS);
            double y = root(f, g, -100, 100, EPS);
            double z = root(f, t, -100, 100, EPS);
            printf("f and g: %lf\n", root(f, g, -100, 100, EPS));
            printf("f and t: %lf\n", root(f, t, -100, 100, EPS));
            printf("g and t: %lf\n", root(g, t, -100, 100, EPS));
            printf("integral f: %lf\n", integral(f, min(y, z), max(y, z), EPS));
            printf("integral g: %lf\n", integral(g, min(x, y), max(x, y), EPS));
            printf("integral t: %lf\n", integral(t, min(x, z), max(x, z), EPS));
        }
        TESTFLAG = 0;
        COUNTFLAG = 0;
        HELPFLAG = 0;
